...
const (
	...
	querySelectGroupByStatus       = "SELECT DATE(created_at), service_id, subject, status, COUNT(msisdn) as count, SUM(ROUND(amount)) as amount FROM transactions WHERE DATE(created_at) >= DATE(NOW() + interval '-7 day') GROUP BY service_id, status, DATE(created_at), subject ORDER BY DATE(created_at) DESC, status DESC, subject ASC LIMIT 50"
	querySelectGroupByStatusDetail = "SELECT DATE(created_at), service_id, subject, status, status_detail, COUNT(msisdn) as count FROM transactions WHERE DATE(created_at) >= DATE(NOW() + interval '-7 day') GROUP BY service_id, status, status_detail, DATE(created_at), subject ORDER BY DATE(created_at) DESC LIMIT 50"
	querySelectGroupByAdnet        = "SELECT DATE(created_at), service_id, adnet, COUNT(msisdn) as count FROM transactions WHERE DATE(created_at) >= DATE(NOW() + interval '-7 day') AND subject = 'FIRSTPUSH' GROUP BY service_id, adnet, DATE(created_at) ORDER BY DATE(created_at) DESC LIMIT 50"
	querySelectTransactionCSV      = "SELECT 'ID' as country, 'telkomsel' as operator, b.name as service, a.channel as source, a.msisdn, a.subject as event, a.created_at as even_date, CONCAT(b.renewal_day, 'd') as cycle, a.amount as revenue, a.updated_at as charge_date, 'IDR' as currency, 'NA' as publisher, 'NA' as handset, 'NA' as browser, a.tx_id as trxid, b.url_telco as telco_api_url, COALESCE(a.payload, 'NA') as telco_api_response, COALESCE(c.value, 'NA') as sms_content, a.status as status_sms, a.camp_sub_keyword FROM public.transactions a LEFT JOIN public.services b ON b.id = a.service_id LEFT JOIN public.contents c ON c.service_id = b.id AND c.name = a.subject WHERE DATE(a.created_at) = DATE(NOW() - interval '1 day') ORDER BY a.id ASC"
	//querySelectTransactionCSV = "SELECT 'ID' as country, 'telkomsel' as operator, b.name as service, a.channel as source, a.msisdn, a.subject as event, a.created_at as even_date, CONCAT(b.renewal_day, 'd') as cycle, a.amount as revenue, a.updated_at as charge_date, 'IDR' as currency, 'NA' as publisher, 'NA' as handset, 'NA' as browser, a.tx_id as trxid, b.url_telco as telco_api_url, COALESCE(a.payload, 'NA') as telco_api_response, COALESCE(c.value, 'NA') as sms_content, a.status as status_sms FROM public.transactions a LEFT JOIN public.services b ON b.id = a.service_id LEFT JOIN public.contents c ON c.service_id = b.id AND c.name = a.subject WHERE DATE(a.created_at) = DATE('2024-03-02') ORDER BY a.id ASC"
)
