...
const (
	...
	queryCountFirstSuccess            = "SELECT COUNT(*) as count FROM subscriptions WHERE service_id = $1 AND msisdn = $2 AND first_success_at IS NULL AND success = 1"
	querySelectSubscription           = "SELECT id, service_id, msisdn, channel, camp_keyword, camp_sub_keyword, adnet, pub_id, aff_sub, latest_trxid, latest_keyword, latest_subject, latest_status, latest_payload, amount, renewal_at, success, ip_address, total_firstpush, total_renewal, total_amount_firstpush, total_amount_renewal, is_retry, is_active, created_at FROM subscriptions WHERE service_id = $1 AND msisdn = $2"
	querySelectAgeDay                 = "SELECT DATE(NOW()) - DATE(created_at) + 1 as count FROM subscriptions WHERE service_id = $1 AND msisdn = $2"
...
type ISubscriptionRepository interface {
	...
	GetAgeDay(int, string) (int, error)
...
func (r *SubscriptionRepository) UpdateSuccess(s *entity.Subscription) error {
	...
	defer stmt.Close()
	res, err := stmt.ExecContext(ctx, s.LatestTrxId, s.LatestSubject, s.LatestStatus, s.LatestPIN, s.Amount, s.RenewalAt, s.ChargeAt, s.Success, s.IsRetry, s.ChargingCount, s.ChargingCountAll, s.TotalFirstpush, s.TotalRenewal, s.TotalAmountFirstpush, s.TotalAmountRenewal, s.LatestPayload, s.ServiceID, s.Msisdn)
	if err != nil {
...
func (r *SubscriptionRepository) UpdateSuccessRetry(s *entity.Subscription) error {
	...
	defer stmt.Close()
	res, err := stmt.ExecContext(ctx, s.LatestTrxId, s.LatestSubject, s.LatestStatus, s.LatestPIN, s.Amount, s.RenewalAt, s.ChargeAt, s.Success, s.Failed, s.IsRetry, s.ChargingCount, s.ChargingCountAll, s.TotalFirstpush, s.TotalRenewal, s.TotalAmountFirstpush, s.TotalAmountRenewal, s.LatestPayload, s.ServiceID, s.Msisdn)
	if err != nil {
		log.Printf("Error %s when update row into subscriptions table", err)
		return err
	}
...
func (r *SubscriptionRepository) CountFirstActive179(serviceId int, msisdn string) (int, error) {
	var count int
	err := r.db.QueryRow(queryCountFirstSuccess, serviceId, msisdn).Scan(&count)
	if err != nil {
		return count, err
...
func (r *SubscriptionRepository) Get(serviceId int, msisdn string) (*entity.Subscription, error) {
	...
	err := r.db.QueryRow(querySelectSubscription, serviceId, msisdn).Scan(&s.ID, &s.ServiceID, &s.Msisdn, &s.Channel, &s.CampKeyword, &s.CampSubKeyword, &s.Adnet, &s.PubID, &s.AffSub, &s.LatestTrxId, &s.LatestKeyword, &s.LatestSubject, &s.LatestStatus, &s.LatestPayload, &s.Amount, &s.RenewalAt, &s.Success, &s.IpAddress, &s.TotalFirstpush, &s.TotalRenewal, &s.TotalAmountFirstpush, &s.TotalAmountRenewal, &s.IsRetry, &s.IsActive, &s.CreatedAt)
	if err != nil {
		return &s, err
	}
	return &s, nil
...
func (r *SubscriptionRepository) GetAgeDay(serviceId int, msisdn string) (int, error) {
	var count int
	err := r.db.QueryRow(querySelectAgeDay, serviceId, msisdn).Scan(&count)
	if err != nil {
		return count, err
	}
	return count, nil
