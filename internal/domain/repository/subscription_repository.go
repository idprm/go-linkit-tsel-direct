...
const (
	...
	querySelectPopulateRetryInsuffBill0 = "SELECT id, service_id, msisdn, channel, adnet, latest_keyword, latest_subject, latest_pin, ip_address, aff_sub, camp_keyword, camp_sub_keyword, retry_at, created_at FROM public.subscriptions WHERE renewal_at IS NOT NULL AND DATE(renewal_at) = DATE(NOW() + interval '1 day') AND DATE(created_at) BETWEEN DATE(NOW() - interval '14 day') AND DATE(NOW()) AND success > failed AND is_retry = true AND is_active = true ORDER BY success DESC, DATE(created_at) DESC"
