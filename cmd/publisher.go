...
var publisherRetryInsuffBill0Cmd = &cobra.Command{
	Use:   "pub_retry_insuff_bill0",
	Short: "Publisher Retry Insuff Bill 0 CLI",
	Long:  ``,
	Run: func(cmd *cobra.Command, args []string) {
		/**
		 * connect pgsql
		...
		for {
			...
			if scheduleService.GetUnlocked(ACT_RETRY_INSUFF_BILL0, timeNow) {

				scheduleService.UpdateSchedule(false, ACT_RETRY_INSUFF_BILL0)

				go func() {
					populateRetryInsuffBill0(db, rmq)
				}()
			}

			if scheduleService.GetLocked(ACT_RETRY_INSUFF_BILL0, timeNow) {
				scheduleService.UpdateSchedule(true, ACT_RETRY_INSUFF_BILL0)
			}
...
var publisherRetryInsuffBill1Cmd = &cobra.Command{
	Use:   "pub_retry_insuff_bill1",
	Short: "Publisher Retry Insuff Bill 1 CLI",
	Long:  ``,
	Run: func(cmd *cobra.Command, args []string) {
		/**
		 * connect pgsql
		...
		for {
			...
			if scheduleService.GetUnlocked(ACT_RETRY_INSUFF_BILL1, timeNow) {

				scheduleService.UpdateSchedule(false, ACT_RETRY_INSUFF_BILL1)

				go func() {
					populateRetryInsuffBill1(db, rmq)
				}()
			}

			if scheduleService.GetLocked(ACT_RETRY_INSUFF_BILL1, timeNow) {
				scheduleService.UpdateSchedule(true, ACT_RETRY_INSUFF_BILL1)
			}
...
var publisherRetryInsuffBill2Cmd = &cobra.Command{
	Use:   "pub_retry_insuff_bill2",
	Short: "Publisher Retry Insuff Bill 2 CLI",
	Long:  ``,
	Run: func(cmd *cobra.Command, args []string) {
		/**
		 * connect pgsql
		...
		for {
			...
			if scheduleService.GetUnlocked(ACT_RETRY_INSUFF_BILL2, timeNow) {

				scheduleService.UpdateSchedule(false, ACT_RETRY_INSUFF_BILL2)

				go func() {
					populateRetryInsuffBill2(db, rmq)
				}()
			}

			if scheduleService.GetLocked(ACT_RETRY_INSUFF_BILL2, timeNow) {
				scheduleService.UpdateSchedule(true, ACT_RETRY_INSUFF_BILL2)
			}
...
var publisherRetryInsuffBill3Cmd = &cobra.Command{
	Use:   "pub_retry_insuff_bill3",
	Short: "Publisher Retry Insuff Bill 3 CLI",
	Long:  ``,
	Run: func(cmd *cobra.Command, args []string) {
		/**
		 * connect pgsql
		...
		for {
			...
			if scheduleService.GetUnlocked(ACT_RETRY_INSUFF_BILL3, timeNow) {

				scheduleService.UpdateSchedule(false, ACT_RETRY_INSUFF_BILL3)

				go func() {
					populateRetryInsuffBill3(db, rmq)
				}()
			}

			if scheduleService.GetLocked(ACT_RETRY_INSUFF_BILL3, timeNow) {
				scheduleService.UpdateSchedule(true, ACT_RETRY_INSUFF_BILL3)
			}
...
func populateRetryInsuffBill0(db *sql.DB, queue rmqp.AMQP) {
	subscriptionRepo := repository.NewSubscriptionRepository(db)
	subscriptionService := services.NewSubscriptionService(subscriptionRepo)

	subs := subscriptionService.RetryInsuffBill0Subscription()
	...
	for _, s := range *subs {
		...
		json, _ := json.Marshal(sub)
		queue.IntegratePublish(RMQ_RETRYINSUFFBILL0EXCHANGE, RMQ_RETRYINSUFFBILL0QUEUE, RMQ_DATATYPE, "", string(json))

		time.Sleep(100 * time.Microsecond)
	}
}

func populateRetryInsuffBill1(db *sql.DB, queue rmqp.AMQP) {
	subscriptionRepo := repository.NewSubscriptionRepository(db)
	subscriptionService := services.NewSubscriptionService(subscriptionRepo)

	subs := subscriptionService.RetryInsuffBill1Subscription()
	...
	for _, s := range *subs {
		...
		json, _ := json.Marshal(sub)
		queue.IntegratePublish(RMQ_RETRYINSUFFBILL1EXCHANGE, RMQ_RETRYINSUFFBILL1QUEUE, RMQ_DATATYPE, "", string(json))

		time.Sleep(100 * time.Microsecond)
	}
}

func populateRetryInsuffBill2(db *sql.DB, queue rmqp.AMQP) {
	subscriptionRepo := repository.NewSubscriptionRepository(db)
	subscriptionService := services.NewSubscriptionService(subscriptionRepo)

	subs := subscriptionService.RetryInsuffBill2Subscription()
	...
	for _, s := range *subs {
		...
		json, _ := json.Marshal(sub)
		queue.IntegratePublish(RMQ_RETRYINSUFFBILL2EXCHANGE, RMQ_RETRYINSUFFBILL2QUEUE, RMQ_DATATYPE, "", string(json))

		time.Sleep(100 * time.Microsecond)
	}
}

func populateRetryInsuffBill3(db *sql.DB, queue rmqp.AMQP) {
	subscriptionRepo := repository.NewSubscriptionRepository(db)
	subscriptionService := services.NewSubscriptionService(subscriptionRepo)

	subs := subscriptionService.RetryInsuffBill3Subscription()
	...
	for _, s := range *subs {
		...
		json, _ := json.Marshal(sub)
		queue.IntegratePublish(RMQ_RETRYINSUFFBILL3EXCHANGE, RMQ_RETRYINSUFFBILL3QUEUE, RMQ_DATATYPE, "", string(json))
